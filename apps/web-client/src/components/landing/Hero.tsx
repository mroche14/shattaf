...
import { ArrowRight, CheckCircle, Clock, MessageCircle, ShieldCheck, Sparkles, Users } from 'lucide-react';
      ...
      <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-8">
          ...
          <p className="text-xl text-[var(--text-secondary)] max-w-xl leading-relaxed font-light">
            ...
            <span className="text-[var(--text-main)] font-medium"> Installation incluse</span> en ~{SITE.offer.installationTimeMinutes} min, partout en Guadeloupe.
          </p>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-5">
              ...
              <button 
                ...
                className="px-6 sm:px-10 py-4 sm:py-5 rounded-2xl glass border-[var(--border-color)] hover:bg-[var(--bg-inner)] transition-all font-bold text-base sm:text-lg uppercase tracking-wider text-[var(--text-main)]"
              >
                Voir la gamme
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-sm">
              <div className="text-[var(--text-secondary)] font-light">
                À partir de <span className="text-[var(--accent-primary)] font-bold">{SITE.offer.minPriceEUR}€</span>
                {SITE.offer.includesInstallation ? <span className="text-[var(--text-tertiary)]"> — installation incluse</span> : null}
              </div>
              {isTruthyString(whatsappHref) && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 text-[var(--accent-primary)] hover:text-[var(--accent-secondary)] font-bold"
                >
          ...
          <div className="flex items-center gap-3 p-4 rounded-2xl bg-[var(--bg-inner)] border border-[var(--border-color)]">
            <Users className="w-5 h-5 text-[var(--accent-primary)] flex-shrink-0" />
            <p className="text-[var(--text-main)] text-sm font-medium">
              Des plombiers professionnels disponibles pour intervenir partout où vous êtes, sans prise de tête.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 pt-10 border-t border-[var(--border-color)]">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-[var(--text-main)] font-bold text-sm">Réponse sous {SITE.offer.responseTime}</span>
                <span className="text-[var(--text-secondary)] text-[11px] uppercase font-black tracking-[0.18em]">Conseil & SAV local</span>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-cyan-500/10 flex items-center justify-center">
              ...
              <div className="flex flex-col">
                <span className="text-[var(--text-main)] font-bold text-sm">Installation ~{SITE.offer.installationTimeMinutes} min</span>
                <span className="text-[var(--text-secondary)] text-[11px] uppercase font-black tracking-[0.18em]">Intervention 971</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative group hidden lg:block">
          <div className="relative glass rounded-[30px] md:rounded-[50px] p-3 md:p-4 animate-float cyan-glow overflow-hidden">
            ...
            <div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10 md:right-10 glass p-5 md:p-8 rounded-[20px] md:rounded-[32px] border-[var(--border-color)]">
