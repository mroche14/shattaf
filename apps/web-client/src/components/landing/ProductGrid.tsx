...
import { Check, Star, Wrench, Shield, Droplets } from 'lucide-react';
import { SITE } from '../../siteConfig';

const ProductGrid: React.FC = () => {
  return (
    <section id="models" className="py-16 md:py-32 bg-[var(--bg-root)]">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-24 gap-8 md:gap-12 text-center md:text-left">
          ...
          <p className="text-gray-400 max-w-md font-light text-lg">
            Chaque modèle intègre un système anti-calcaire et un dispositif anti-retour conforme aux normes européennes NF/CE.
          </p>
        </div>
            ...
            <div key={product.id} className="group flex flex-col h-full glass rounded-[24px] md:rounded-[50px] overflow-hidden border-white/5 hover:border-cyan-500/30 transition-all duration-700">
              ...
              <div className="p-6 md:p-10 space-y-6 md:space-y-10 flex-1 flex flex-col">
                ...
                <div className="flex flex-wrap gap-2">
                  <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-[10px] font-bold uppercase tracking-wider text-cyan-300">
                    <Droplets className="w-3.5 h-3.5" /> Anti-calcaire
                  </div>
                  <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-bold uppercase tracking-wider text-emerald-300">
                    <Shield className="w-3.5 h-3.5" /> Anti-retour NF/CE
                  </div>
                </div>
